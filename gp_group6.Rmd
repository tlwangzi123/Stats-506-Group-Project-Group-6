...
*SexI* has a estimate coefficient -0.14064, and *SexI* is significant(because the p-value of *SexI* is 3.28e-10 < 0.05), 
which means the log count of Rings for a Infant Abalone is obviously less compared to Female Abalone by 0.14064 when the length of these Abalones are the same.    
*SexM* has a estimate coefficient 0.02083, and *SexM* is not significant(because the p-value of *SexM* is 0.232 > 0.05), which means the log count of Rings for a Infant Abalone is inconspicuously less compared to Female Abalone by 0.02083 when the length of these Abalones are the same.  
*Length* has a estimate coefficient 1.50445, and *Length* is not significant(because the p-value of *Length* < 2e-16 < 0.05), which means the log count of Rings has a obviously increase by 1.50445 for each mm increase in Longest shell measurement when the sex keeps the same.
The first intercept((Intercept):1) is 1.63153, this shows that the log count of the Rings of the Abalone 1.63153 when all predictors equal zero.  
The second intercept((Intercept):2) is 5.19065, which means the value of
the over dispersion parameter *k* in the Zero-Truncated Negative Binomial Regression model is 5.19065.
...
From this plot, we can see: the Rings for a Infant Abalone is obviously less compared to Female Abalone when the length of these Abalones are the same, while the Rings for a Infant Abalone is inconspicuously less compared to Female Abalone when the length of these Abalones are the same. This is consistent with the conclusion in the interpretion part.
...
## Things to Consider
From the example above, we can see that the coefficient of Zero-Truncated Negative Binomial Regression in SAS part has some differences between that in R and Stata. Because it may cause some accuracy errors when to use iteration method in SAS to maximize the log-likelihood function in order to find the estimators because of limitation in R in the convergence procedure.  
Besides, we can see that even if the Log-likelihood criterion 
